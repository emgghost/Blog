...
<template>
  <v-app>
    ...
    <v-main class="!bg-[#edf3f5]">
      <swiper :navigation="true" :modules="modules" class="mySwiper !h-[450px] mt-[50px]" v-if="route.path === '/'">
        <swiper-slide v-for="post in posts.filter((item)=>item.addToSlider===true)">
          <img :alt="post.title" :title="post.title" :src="!!post.sliderImageUrl? fileUrl + post.sliderImageUrl : fileUrl + post.imageUrl" class="h-[370px] w-full rounded-t-lg group-hover:!scale-110 delay-3s duration-500 transition-all"/>
