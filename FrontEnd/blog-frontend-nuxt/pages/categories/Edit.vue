  ...
  methods: {
    async fetchCategory() {
      ...
      try {
        ...
        this.category = response.data; // This should include the id
      } catch (error) {
    ...
    async submitForm() {
      try {
        const response = await this.$axios.put(`/categories/${this.category.id}`, this.category); // Ensure category.id is used here
